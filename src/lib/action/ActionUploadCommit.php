...
class ActionUploadCommit extends AActionUploadId {
  ...
  public function run($req) {
    ...
    if (!$this->getFS()->fsFileExists(TRUE, $dir) && !$this->getFS()
        ->fsMkDir(TRUE, $dir, 0777, TRUE)) {
      throw new MessageException(
        Message::createMessage(Message::DIR_DOES_NOT_EXIST, $req->dir)
      );
    }
    ...
    for ($i = 0; $i < count($filesToCommit); $i++) {
      ...
      $cachedFile = $this->m_config->getFS()
        ->getCachedFile($this->m_config->getBaseDir() . "/" . $fileCommited->getPath());
      $cachedFile->delete();
      ...
      if ($req->imageFormats != NULL) {
        ...
        foreach ($req->imageFormats as $imageFormat) {
          ...
          try {
            ...
            if ($e->getFailMessage()["code"] !== Message::FM_NOT_ERROR_NOT_NEEDED_TO_UPDATE) {
